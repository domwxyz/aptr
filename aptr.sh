...
# Default: strongly prefer stable packages (but rolling wins)
Package: *
...
install_package() {
    ...
    if [[ "$from_unstable" == true ]]; then
        ...
        if dpkg-query -W -f='${Status}' "$package" 2>/dev/null | grep -q "install ok installed"; then
            # Package is installed - check if it's already rolling
            if grep -q "^$package$" "$ROLLING_PACKAGES_FILE" 2>/dev/null; then
                log_info "Package '$package' is already rolling. Upgrading..."
                # Just upgrade the existing rolling package
                ...
                if [[ "$DRY_RUN" == true ]]; then
                    log_info "[DRY RUN] Would upgrade rolling package $package"
                    return 0
                fi
                
                check_apt_lock || return 1
                if DEBIAN_FRONTEND=noninteractive $apt_cmd "$package"; then
                    log_success "Successfully upgraded rolling package $package"
                else
                    log_error "Failed to upgrade $package"
                    return 1
                fi
            else
                # Package installed but not rolling - convert it
                log_info "Package '$package' is installed from stable. Converting to rolling..."
                if [[ "$DRY_RUN" == true ]]; then
                    log_info "[DRY RUN] Would convert $package to rolling and upgrade"
                    return 0
                fi
                
                # Use the existing roll_package logic
                roll_package "$package"
                return $?
            fi
        else
            # Package not installed - fresh install from unstable
        ...
        # Install from stable (existing logic unchanged)
...
roll_package() {
    ...
    # Compare versions to ensure unstable is actually newer
    local current_epoch=$(echo "$current_version" | grep -o '^[0-9]*:' | tr -d ':')
    local unstable_epoch=$(echo "$unstable_version" | grep -o '^[0-9]*:' | tr -d ':')

    # Simple version comparison (handles most cases)
    if [[ -n "$unstable_version" && "$unstable_version" != "Unknown" ]]; then
        if dpkg --compare-versions "$current_version" ge "$unstable_version"; then
            log_warning "Current version ($current_version) is not older than unstable ($unstable_version)"
            if ! confirm_action "Continue anyway? (This may not upgrade the package)"; then
                log_info "Rolling cancelled"
                return 0
            fi
        fi
    fi
